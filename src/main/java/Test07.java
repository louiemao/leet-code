...
public class Test07 {
        ...
        try {
            ...
            for (int i = 0; i < ids.length; i++) {
                ...
                if (i == ids.length - 1) {
                    ...
                    length = write(writeFilePath, length, stringBuffer.toString().getBytes(StandardCharsets.UTF_8));
    ...
    private static long write(String to, long offset, byte[] bs) throws IOException {
        File f = new File(to);
        // 文件/目录不存在则将文件/目录创建
        if (!f.exists()) {
            if (!f.getParentFile().exists()) {
        ...
        if (f.length() != offset) {
            System.out.println("offset:" + offset + ", f.length():" + f.length());
        }
    ...
    static class MyRunnable implements Runnable {
        ...
        public void run() {
            ...
            try {
                reader = new MappedBiggerFileReader(filePath, "UTF-8", 65536);
